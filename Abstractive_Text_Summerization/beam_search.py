...
def run_beam_search(sess, model, vocab, batch):
    ...
    steps = 0
    max_dec_steps=FLAGS.max_dec_steps if FLAGS.mode!="predict" else batch.max_dec_steps
    while steps < max_dec_steps and len(results) < FLAGS.beam_size:
        ...
        for i in range(num_orig_hyps):
            h, new_state, attn_dist, p_gen, new_coverage_i = hyps[i], new_states[i], attn_dists[i], p_gens[i], \
                                                             new_coverage[i]  # take the ith hypothesis and new decoder state info
        ...
        cnt_trigram = 0
        min_dec_steps=FLAGS.min_dec_steps if FLAGS.mode!="predict" else batch.min_dec_steps

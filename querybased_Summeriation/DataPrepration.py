...
def main():
    ...
    train1 = flatten_json(args.trn_file, 'train')
    dev1 = flatten_json(args.dev_file, 'dev')
    train2 = flatten_json(args.trn2_file, 'train')
    dev2 = flatten_json(args.dev2_file, 'dev')
    train=train1+train2
    dev=dev1+dev2
...
def setup():
    ...
    parser.add_argument('--trn_file', default='SQuAD/data/train-v1.1.json',
                        help='path to train file.')
    parser.add_argument('--dev_file', default='SQuAD/data/dev-v1.1.json',
                        help='path to dev file.')
    parser.add_argument('--trn2_file', default='SQuAD/data/train-v2.0.json',
                        help='path to train file.')
    parser.add_argument('--dev2_file', default='SQuAD/data/dev-v2.0.json',
                        help='path to dev file.')
        ...
        for paragraph in article['paragraphs']:
            ...
            for qa in paragraph['qas']:
                ...
                if mode == 'train':
                    ...
                    rows.append((id_, context.casefold(), question.casefold(), answer.casefold(), answer_start, answer_end))
                else:  # mode == 'dev'
                    answers = [a['text'].casefold() for a in answers]
                    rows.append((id_, context.casefold(), question.casefold(), answers))
    return rows
